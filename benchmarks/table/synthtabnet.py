...
class GroundTruthPagesForcer:
    def __init__(self):
        # debug data: [convert_to_page(get_raw_test_data(), [0, 0, 612.0, 792.0], 0)]
        self.forced_pages = None
    def __call__(self, filepath):
        return self.forced_pages

class ChangedTableProcessor(TableProcessor):
    """
    Works when only the image and text are available, and PDF is not.
    """
...
class SynthTabNetBenchmark(FinTabNetBenchmark):
    ...
    def get_converter(self, models, config_parser, **kwargs):
        ...
        return ChangedTableConverter(
            ...
            processor_list=[
                "benchmarks.table.synthtabnet.ChangedTableProcessor",
